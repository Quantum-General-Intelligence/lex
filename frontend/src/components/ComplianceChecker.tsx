...
import { Upload, AlertTriangle, CheckCircle, AlertCircle, FileText, Loader2, Shield, Download } from 'lucide-react'
import { DEMO_COMPLIANCE_RESPONSE } from '@/constants/demoData'
import { API_BASE_URL } from '@/lib/api'
import { ComplianceResultSkeleton } from '@/components/Skeleton'
...
export function ComplianceChecker() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE_URL}/api/analysis/compliance`, {
        method: 'POST',
  ...
  const exportReport = () => {
    if (!response) return
    const content = `COMPLIANCE CHECK REPORT
Generated: ${new Date().toLocaleString()}

Status: ${getStatusConfig(response.overall_status).label}
Regulations Checked: ${response.checked_regulations}
Processing Time: ${response.processing_time_ms.toFixed(0)}ms

${response.document_summary}

${response.issues.length > 0 ? `ISSUES FOUND (${response.issues.length}):
${response.issues.map((issue, i) => `
${i + 1}. [${issue.severity.toUpperCase()}] ${issue.regulation_citation}
...
Users may request access to their records by contacting the agency.`)}
      ...
      {/* Loading State */}
      {loading && <ComplianceResultSkeleton />}
          ...
          <div className={`p-4 rounded-xl ${getStatusConfig(response.overall_status).bg} border ${getStatusConfig(response.overall_status).border}`}>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 text-vulcan-400 hover:text-white hover:bg-vulcan-700 rounded-lg transition-colors"
                title="Export report"
              >
